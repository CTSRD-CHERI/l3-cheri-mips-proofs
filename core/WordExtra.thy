...
imports 
  ...
  "$ISABELLE_HOME/src/HOL/Lattice/Lattice"
begin

(*>*)
section \<open>Library for machine words\<close>

text \<open>This theory contains various lemmas about machine words.\<close>
...
subsection \<open>Powers of two\<close>

lemma uint_power_two:
  shows "uint ((2::'a::len word) ^ n) = (if LENGTH('a) \<le> n then 0 else 2 ^ n)"
proof (cases "LENGTH('a) \<le> n")
  case True
  define m where "m \<equiv> n - LENGTH('a)"
  have n_alt: "n = m + LENGTH('a)"
    unfolding m_def
    using True
    by simp
  have "(2::'a word) ^ n = 0"
    unfolding n_alt power_add word_pow_0
    by simp
  thus ?thesis
    using True by simp
next
  case False
  hence "0 < (2::'a word) ^ n"
    using word_2p_lem[where w="0::'a word" and n=n]
    unfolding word_size 
    by simp
...
corollary unat_power_two:
  shows "unat ((2::'a::len word) ^ n) = (if LENGTH('a) \<le> n then 0 else 2 ^ n)"
unfolding unat_def uint_power_two
by (simp add: nat_power_eq)

lemma word_power_two_eq_zero [simp]:
  shows "((2::'a::len word) ^ n = 0) = (LENGTH('a) \<le> n)"
...
lemma mask_eq_mask [simp]:
  shows "((mask n::'a::len word) = mask m) = (LENGTH('a) \<le> min m n \<or> m = n)"
proof
  assume "(mask n::'a word) = mask m"
  from test_bit_cong[OF this, where x=n]
       test_bit_cong[OF this, where x=m]
  show "LENGTH('a) \<le> min m n \<or> m = n"
    by (auto simp: word_size)
qed auto

corollary mask_eq_max_word [simp]:
  shows "((mask n::'a::len word) = max_word) = (LENGTH('a) \<le> n)"
unfolding max_word_alt_def
by (auto simp del: mask_max_word)

corollary max_word_eq_mask [simp]:
  shows "(max_word = (mask n::'a::len word)) = (LENGTH('a) \<le> n)"
unfolding max_word_alt_def
by (auto simp del: mask_max_word)
...
lemma max_word_wrap_eq:
  fixes x :: "'a::len word"
  shows "(x + 1 = 0) = (x = max_word)"
using max_word_wrap
unfolding max_word_minus
by auto
...
proof (cases "n \<le> LENGTH('a)")
  ...
  have [simp]: "((2::int) ^ LENGTH('a) - 1) div 2 ^ n = 2 ^ LENGTH('a) div 2 ^ n - 1"
    using zdiv_zadd1_eq[where a="2 ^ LENGTH('a)" and b="-1"]
...
lemma word_and_not_mask_and_not_mask [simp]:
  shows "(x AND NOT mask n) AND NOT mask m = x AND NOT mask (max n m)"
by (intro word_eqI) (auto simp: word_size word_ops_nth_size)
...
lemma slice_word_and_not_mask [simp]:
  fixes x :: "'a::len word"
  assumes " m \<le> n"
  shows "(slice n (x AND NOT mask m)::'b::len0 word) = slice n x"
proof (intro word_eqI impI, simp only: word_size)
  fix i
  assume "i < LENGTH('b)"
  thus "(slice n (x AND NOT mask m)::'b word) !! i = (slice n x::'b word) !! i"
    using assms Word.test_bit_size[where w=x and n="i + n"]
    by (auto simp add: nth_slice word_ao_nth word_ops_nth_size word_size) 
qed

lemma and_not_mask_slice_beyond_length [simp]:
  fixes x :: "'a::len word"
  assumes "LENGTH('a) \<le> n + m"
  shows "(slice n x) AND NOT mask m = 0" (is "?l = ?r")
proof (intro word_eqI impI, unfold word_size)
  fix i
  assume "i < LENGTH('b)"
  thus "?l!! i = ?r !! i"
    using assms test_bit_size[where w=x and n="i + n"]
    by (auto simp add: nth_slice word_ops_nth_size word_size) 
qed

corollary and_not_mask_ucast_beyond_length [simp]:
  fixes x :: "'a::len word"
  assumes "LENGTH('a) \<le> m"
  shows "(ucast x) AND NOT mask m = 0" (is "?l = ?r")
using assms and_not_mask_slice_beyond_length[where n=0]
unfolding slice_zero[THEN sym]
by (simp del: slice_zero)
...
lemma ucast_power_two:
  shows "ucast ((2::'a::len word) ^ n) = 2 ^ n AND mask LENGTH('a)"
by (intro word_eqI)
   (simp add: word_size nth_ucast word_ao_nth nth_w2p)
...
lemma slice_word_cat_zero [simp]:
  fixes x :: "'a::len word"
  assumes "LENGTH('b) \<le> n"
      and "LENGTH('c) = LENGTH('a) + LENGTH('b)"
  shows "(slice n (word_cat x (0::'b::len0 word)::'c::len0 word)::'d::len0 word) = 
         slice (n - LENGTH('b)) x" 
         (is "?l = ?r")
proof (intro word_eqI impI, unfold word_size)
  fix i
  ...
  show "?l !! i = ?r !! i"
    using assms
    using test_bit_size[where w=x and n="i + n - LENGTH('b)"]
    by (auto simp: word_size nth_word_cat nth_slice word_ops_nth_size)
qed
