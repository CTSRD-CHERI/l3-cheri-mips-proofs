...
lemma ExceptionCase_NoException:
  ...
  shows "PrePost q m (\<lambda>_. exceptionLemmaPost exl pc pcc capr scapr mem)"
using assms
...
method ExceptionCase =
  ParametricPrePost
      \<open>ComputePre \<open>rule ExceptionCase_NoException_loops; 
                   ...
                   rule ExceptionCases[THEN PrePost_post_weakening];
                      solves \<open>auto split: all_split simp add: ValueAndStatePart_simp\<close>\<close>
                  \<open>rule PrePost_weakest_pre_read_state |
...
lemma SignalException_getExceptionOccurredPrePost:
...
lemma ExceptionCase_SignalException [ExceptionCases]:
  shows "PrePost (read_state (getExceptionOccurredPre exl pc pcc capr scapr mem)) 
                 (SignalException v)
                 (\<lambda>_. read_state getExceptionSignalled \<and>\<^sub>b
                      read_state (getExceptionOccurredPost exl pc pcc capr scapr mem))"
unfolding PrePost_def
by (auto simp: ValueAndStatePart_simp
         elim: SignalException_getExceptionOccurredPrePost)
...
by (PrePost intro: ExceptionCase_SignalException)
...
by (PrePost intro: ExceptionCase_SignalCapException_internal)
...
by (PrePost intro: ExceptionCase_SignalCapException_noReg)
...
by (PrePost intro: ExceptionCases[THEN PrePost_post_weakening])
...
proof -
  ...
  thus ?thesis
    using assms
    using PrePostE[OF ExceptionCase_NextWithGhostState, where s=s]
    by (auto simp: ValueAndStatePart_simp)
qed
