...
class ValidStateInvariantGenerator(CodeGenerator):

  def __init__(self):
    CodeGenerator.__init__(self, "StateIsValid invariant")
  ...
  def functionToIsa(self, l3Function):
    if (isinstance(l3Function, UnfoldFun)):
      ...
      result = \
"""lemma ValidStateInvariant_[NAME] [ValidStateInvariantI]:
  shows "IsInvariant ValidStateInvariant [NAME+VARS]" 
